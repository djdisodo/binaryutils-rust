...
pub trait BinaryStream {
	...
	fn set_offset(&mut self, offset: usize) {
		self.get_binary_stream_mut().offset = offset;
	}
	fn set_buffer(&mut self, buffer: Vec<u8>, offset: usize) {
		self.get_binary_stream_mut().buffer = buffer;
		self.set_offset(offset);
	}
	fn get_offset(&self) -> usize {
		return self.get_binary_stream_ref().offset;
	}
	fn get_buffer(&self) -> &Vec<u8> {
		return &self.get_binary_stream_ref().buffer;
	}
	fn get(&mut self, len: usize) -> Vec<u8> {
		...
		result.resize(len, 0);
		self.get_binary_stream_mut().offset.add_assign(1);
		return result;
	}
	...
	fn put(&mut self, bytes: Vec<u8>) {
		self.get_binary_stream_mut().buffer.extend(bytes);
	}
	fn get_bool(&mut self) -> bool {
		return self.get(1).as_slice()[self.get_offset()] != 0;
	}
	fn put_bool(&mut self, v: bool) {
		self.get_binary_stream_mut().buffer.push(v as u8);
	}
	fn get_byte(&mut self) -> u8 {
		return self.get(1).get(0).unwrap().clone();
	}
	fn put_byte(&mut self, v: u8) {
		self.get_binary_stream_mut().buffer.push(v);
	}
	...
	fn get_unsigned_var_int(&mut self, endian: Endian) -> u32 {
		...
		self.get_binary_stream_mut().offset.add(read_bytes as usize);
		return result;
	}
